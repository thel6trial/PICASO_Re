...
class Seq2Seq(nn.Module):
    ...
    def __init__(self, encoder, decoder, config, beam_size=None, max_length=None, \
        sos_id=None, eos_id=None, l2_norm=False, fusion=True):
        ...
        if self.fusion_flag == True:    
            self.fusion1 = nn.Linear(config.hidden_size * 3, config.hidden_size)
            self.fusion2 = nn.Linear(config.hidden_size * 3, config.hidden_size)
            self.fusion3 = nn.Linear(config.hidden_size * 3, config.hidden_size)
            self.fusion = nn.Linear(config.hidden_size * 4, config.hidden_size)
        else:
            self.fusion_single = nn.Linear(
    ...
    def forward(self,   
        src_ant_ids=None, src_ant_mask=None, \
        so_title_ids=None, so_title_mask=None, \
        so_api_ids=None, so_api_mask=None, \
        ...
        so_title_outputs = self.encoder(so_title_ids, attention_mask=so_title_mask)
        so_title_encoder_output = so_title_outputs[0].permute([1, 0, 2]).contiguous()

        so_api_outputs = self.encoder(so_api_ids, attention_mask=so_api_mask)
        so_api_encoder_output = so_api_outputs[0].permute([1, 0, 2]).contiguous()
        ...
        if self.l2_norm == True:
            ...
            so_title_encoder_output = F.normalize(so_title_encoder_output, p=2, dim=-1)
            so_api_encoder_output = F.normalize(so_api_encoder_output, p=2, dim=-1)
            
        if self.fusion_flag == True:
            output_1 = self.fusion1(
                torch.cat([                    
                    src_ant_encoder_output,
                    so_title_encoder_output,
                    so_api_encoder_output
                ], dim=-1))
            
            output_2 = self.fusion2(
                    torch.cat([
                    src_ant_encoder_output, \
                    src_ant_encoder_output - \
                    so_title_encoder_output, \
                    src_ant_encoder_output - so_title_encoder_output - so_api_encoder_output
                    ], dim=-1))
            ...
            encoder_output = self.dropout(self.fusion_single(torch.cat([ \
                src_ant_encoder_output, \
                so_title_encoder_output, \
                so_api_encoder_output
            ], dim=-1)))
        ...
        if target_ids is not None:
            ...
            for i in range(so_title_ids.shape[0]):
                context = encoder_output[:, i:i + 1]
                context_mask = so_title_mask[i:i + 1, :]
...
class OrthrusDualAPI():
    def __init__(self, codebert_path, decoder_layers, fix_encoder, beam_size, \
        ...
        logging.info("loading model from {}".format(load_model_path))
        logging.info("l2 normalization: {}".format(l2_norm))
        logging.info("fusion: {}".format(fusion))
    ...
    def train(self,
        ...
        all_so_title_ids = torch.tensor([f.so_title_ids for f in train_features], dtype=torch.long)
        all_so_title_mask = torch.tensor([f.so_title_mask for f in train_features], dtype=torch.long)

        all_so_api_ids = torch.tensor([f.so_api_ids for f in train_features], dtype=torch.long)
        all_so_api_mask = torch.tensor([f.so_api_mask for f in train_features], dtype=torch.long)
        ...
        train_data = TensorDataset(            
            ...
            all_so_title_ids, all_so_title_mask, \
            all_so_api_ids, all_so_api_mask, \
        ...
        for epoch in range(num_train_epochs):
            ...
            for batch in bar:
                ...
                so_title_ids, so_title_mask, \
                so_api_ids, so_api_mask, \
                target_ids, target_mask = batch
                
                loss, _, _ = self.model(                    
                    src_ant_ids=src_ant_ids, \
                    src_ant_mask=src_ant_mask, \
                    so_title_ids=so_title_ids, \
                    so_title_mask=so_title_mask, \
                    so_api_ids=so_api_ids, \
                    so_api_mask=so_api_mask, \
                ...
                nb_tr_examples += so_title_ids.size(0)
                nb_tr_steps += 1
                loss.backward()
            ...
            if do_eval==True:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    all_so_title_ids = torch.tensor([f.so_title_ids for f in eval_features], dtype=torch.long)
                    all_so_title_mask = torch.tensor([f.so_title_mask for f in eval_features], dtype=torch.long)

                    all_so_api_ids = torch.tensor([f.so_api_ids for f in eval_features], dtype=torch.long)
                    all_so_api_mask = torch.tensor([f.so_api_mask for f in eval_features], dtype=torch.long)
                ...
                df_annotation = pd.DataFrame({'annotation': [example.src_ant for example in eval_examples]})

                # Add the targetAPI and predicted_api columns
                df_annotation['targetAPI'] = pd.DataFrame({'target': [example.target for example in eval_examples]})
            ...
            for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                ...
                with torch.no_grad():
                    preds = self.model(                        
                        ...
                        so_title_ids=so_title_ids, so_title_mask=so_title_mask , \
                        so_api_ids=so_api_ids, so_api_mask=so_api_mask
                    )
    ...
    def predict(self, so_title, so_api, src_ant):
        encode = self.tokenizer.encode_plus(so_title, return_tensors="pt", \
            max_length=self.max_source_length, truncation=True, \
            pad_to_max_length=True)        
        so_title_ids = encode['input_ids'].to(self.device)
        so_title_mask = encode['attention_mask'].to(self.device)

        encode = self.tokenizer.encode_plus(so_api, return_tensors="pt", \
            max_length=self.max_source_length, truncation=True, \
            pad_to_max_length=True)        
        so_api_ids = encode['input_ids'].to(self.device)
        so_api_mask = encode['attention_mask'].to(self.device)
